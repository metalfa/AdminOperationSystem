...
class Console:
  def type(words,secs=None):
    if type(secs) == str:
      try:
        secs = float(secs)
        raise ValueError("[WARN] AOS requested int or float for Console.type. Autoconverted str.")
      except Exception as e:
        raise ValueError("AOS requested int or float or float as a string for Console.type. An invalid "+str(type(1)).replace("<class '", "").replace("'>","")+" was passed: "+str(secs))
    if secs == None:
      wait = 0.1
    else:
      wait = secs
...
class System:
  ...
  def setProjPoint(files):
    if os.path.isfile(files) == True:
      ...
      raise FileNotFoundError(f"File {files} is not found by Pythus in this repl's directory, make sure you put the directory name with a '/' as  well if it belongs in directories")
    file = open(".replit","a+")
...
class aosgraphix:
  def importGraphics():
    try:
      ...
      #^pyflakes
      print("Copy over the graphics_engine.py file to your project, then retry this command.")
